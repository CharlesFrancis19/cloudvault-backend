...
app.get('/files/stats', async (req, res) => {
  try {
    ...
    if (scope === 'all') {
      ...
      if (!hdr?.startsWith('Bearer ')) return res.status(401).json({ error: 'Missing bearer token' });
      try {
        const user = jwt.verify(hdr.slice(7), JWT_SECRET);
        prefix = `users/${(user?.email || 'anon').toLowerCase()}/`;
      } catch {
    ...
    res.json({ totalFiles, totalBytes, scope: scope === 'all' ? 'all' : 'me' });
  } catch (err) {
    console.error('Error fetching S3 stats:', err);
